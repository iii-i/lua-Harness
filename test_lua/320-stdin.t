...
function foo (x)
    ...
    local cmd = lua .. [[ -e "f = loadfile(); print(foo); f(); print(foo('ok'))" < foo.lua]]
    f = io.popen(cmd)
...
do
    ...
    local cmd = lua .. [[ -e "print(io.read'*l'); print(io.read'*l'); print(io.type(io.stdin))" < file.txt]]
    f = io.popen(cmd)
...
do
    local f = io.open('number.txt', 'w')
    f:write("6.0     -3.23   15e3\n")
    f:write("4.3     234     1000001\n")
    f:close()

    local cmd = lua .. [[ -e "while true do local n1, n2, n3 = io.read('*number', '*number', '*number'); if not n1 then break end; print(math.max(n1, n2, n3)) end" < number.txt]]
    f = io.popen(cmd)
...
do
    local f = io.open('dbg.txt', 'w')
    f:write("print 'ok'\n")
    f:write("error 'dbg'\n")
    f:write("cont\n")
    f:close()

    local cmd = lua .. [[ -e "debug.debug()" < dbg.txt]]
    f = io.popen(cmd)
