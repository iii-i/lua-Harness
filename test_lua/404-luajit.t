#! /usr/bin/lua
--
-- lua-Harness : <https://fperrad.frama.io/lua-Harness/>
--
-- Copyright (C) 2018, Perrad Francois
--
-- This code is licensed under the terms of the MIT/X11 license,
-- like Lua itself.
--

--[[

=head1 LuaJIT Stand-alone

=head2 Synopsis

    % prove 404-luajit.t

=head2 Description

See L<http://luajit.org/running.html>

=cut

--]]

require'tap'

if not jit then
    skip_all("only with LuaJIT")
end

local lua = arg[-3] or arg[-1]

if not pcall(io.popen, lua .. [[ -e "a=1"]]) then
    skip_all("io.popen not supported")
end

local compiled_with_jit = jit.status()

plan'no_plan'
diag(lua)

local f = io.open('hello.lua', 'w')
f:write([[
print 'Hello World'
]])
f:close()

os.execute(lua .. " -b hello.lua hello.out")
local cmd = lua .. " hello.out"
f = io.popen(cmd)
is(f:read'*l', 'Hello World', "-b")
f:close()

os.execute(lua .. " -bg hello.lua hello.out")
cmd = lua .. " hello.out"
f = io.popen(cmd)
is(f:read'*l', 'Hello World', "-bg")
f:close()

os.execute(lua .. " -be 'print[[Hello World]]' hello.out")
cmd = lua .. " hello.out"
f = io.popen(cmd)
is(f:read'*l', 'Hello World', "-be")
f:close()

os.remove('hello.out') -- clean up

cmd = lua .. " -bl hello.lua"
f = io.popen(cmd)
like(f:read'*l', '^%-%- BYTECODE %-%- hello%.lua', "-bl hello.lua")
like(f:read'*l', '^0001    %u[%u%d]+%s+')
like(f:read'*l', '^0002    %u[%u%d]+%s+')
like(f:read'*l', '^0003    %u[%u%d]+%s+')
f:close()

os.execute(lua .. " -bl hello.lua hello.txt")
f = io.open('hello.txt', 'r')
like(f:read'*l', '^%-%- BYTECODE %-%- hello%.lua', "-bl hello.lua hello.txt")
like(f:read'*l', '^0001    %u[%u%d]+%s+')
like(f:read'*l', '^0002    %u[%u%d]+%s+')
like(f:read'*l', '^0003    %u[%u%d]+%s+')
f:close()

os.remove('hello.txt') -- clean up

os.execute(lua .. " -b hello.lua hello.c")
f = io.open('hello.c', 'r')
like(f:read'*l', '^#ifdef _cplusplus$', "-b hello.lua hello.c")
like(f:read'*l', '^extern "C"$')
like(f:read'*l', '^#endif$')
like(f:read'*l', '^#ifdef _WIN32$')
like(f:read'*l', '^__declspec%(dllexport%)$')
like(f:read'*l', '^#endif$')
like(f:read'*l', '^const.- char luaJIT_BC_hello%[%] = {$')
like(f:read'*l', '^%d+,%d+,%d+,')
f:close()

os.remove('hello.c') -- clean up

os.execute(lua .. " -b hello.lua hello.h")
f = io.open('hello.h', 'r')
like(f:read'*l', '^#define luaJIT_BC_hello_SIZE %d+$', "-b hello.lua hello.h")
like(f:read'*l', '^static const.- char luaJIT_BC_hello%[%] = {$')
like(f:read'*l', '^%d+,%d+,%d+,')
f:close()

os.remove('hello.h') -- clean up

cmd = lua .. " -j flush hello.lua"
f = io.popen(cmd)
is(f:read'*l', 'Hello World', "-j flush")
f:close()

cmd = lua .. " -joff hello.lua"
f = io.popen(cmd)
is(f:read'*l', 'Hello World', "-joff")
f:close()

cmd = lua .. " -jon hello.lua 2>&1"
f = io.popen(cmd)
if compiled_with_jit then
    is(f:read'*l', 'Hello World', "-jon")
else
    like(f:read'*l', "^[^:]+: JIT compiler permanently disabled by build option", "no jit")
end
f:close()

cmd = lua .. " -j bad hello.lua 2>&1"
f = io.popen(cmd)
like(f:read'*l', "^[^:]+: unknown luaJIT command or jit%.%* modules not installed", "-j bad")
f:close()

if compiled_with_jit then
    cmd = lua .. " -O hello.lua"
    f = io.popen(cmd)
    is(f:read'*l', 'Hello World', "-O")
    f:close()

    cmd = lua .. " -O3 hello.lua"
    f = io.popen(cmd)
    is(f:read'*l', 'Hello World', "-O3")
    f:close()

    cmd = lua .. " -Ocse -O-dce -Ohotloop=10 hello.lua"
    f = io.popen(cmd)
    is(f:read'*l', 'Hello World', "-Ocse -O-dce -Ohotloop=10")
    f:close()

    cmd = lua .. " -O+cse,-dce,hotloop=10 hello.lua"
    f = io.popen(cmd)
    is(f:read'*l', 'Hello World', "-O+cse,-dce,hotloop=10")
    f:close()

    cmd = lua .. " -O+bad hello.lua 2>&1"
    f = io.popen(cmd)
    like(f:read'*l', "^[^:]+: unknown or malformed optimization flag '%+bad'", "-O+bad")
    f:close()
else
    cmd = lua .. " -O0 hello.lua 2>&1"
    f = io.popen(cmd)
    like(f:read'*l', "^[^:]+: attempt to index a nil value")
    f:close()
end

os.remove('hello.lua') -- clean up

done_testing()

-- Local Variables:
--   mode: lua
--   lua-indent-level: 4
--   fill-column: 100
-- End:
-- vim: ft=lua expandtab shiftwidth=4:
